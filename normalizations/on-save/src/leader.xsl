  ...
  <xsl:template match="leader">
    ...
    <xsl:variable name="pos19">
      <xsl:choose>
        ...
        <xsl:when test="../datafield[@tag='773'][subfield[@code='w']/text()][not(subfield[@code='i']/text())]">c</xsl:when>
        <xsl:when test="../datafield[@tag='830']/subfield[@code='w']/text()">b</xsl:when>
        <xsl:when test="../datafield[@tag='773'][subfield[@code='i']/text()]">{' '}</xsl:when>
